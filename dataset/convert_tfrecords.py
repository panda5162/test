...
'''How to organize your dataset folder:
  VOCROOT/
       ...
       |    |->...
'''
tf.app.flags.DEFINE_string('dataset_directory', 'VOCROOT',
                           'All datas directory')
...
class ImageCoder(object):
  ...
  def __init__(self):
    ...
    image = tf.image.decode_png(self._png_data, channels=3)  #解析，得到image的tensor形式.二进制文件解码为uint8.每个数字都为0-255之间的数字
...
def _process_image(filename, coder):
  ...
  # Decode the RGB JPEG.
  image = coder.decode_jpeg(image_data)  #矩阵形式
...
def _process_image_files_batch(coder, thread_index, ranges, name, directory, all_records, num_shards):
  ...
  shard_ranges = np.linspace(ranges[thread_index][0],
                             ...
                             num_shards_per_batch + 1).astype(int)  #指定开始值、终值和元素个数创建表示等差数列的一维数组
